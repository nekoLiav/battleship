...
if (randomOrientation === 0) {
  for (let i = 0; i < player.boardArray.length; i++) {
    if (player.boardArray[i][0] <= ship.shipArray.length) {
...
if (randomOrientation === 1) {
  for (let i = 0; i < player.boardArray.length; i++) {
    if (player.boardArray[i][1] <= ship.shipArray.length) {
      for (let n = 0, y = 0; n < ship.shipArray.length; n++) {
        if (player.boardArray[i + (n + 10)].length === 3) {
          y++
        }
