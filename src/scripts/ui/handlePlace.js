...
export default function handlePlace(e) {
  ...
  if (
    ...
    if (selectedShipSelector.classList.contains('horizontal')) {
      if (manualX <= 10 - selectedShip.length) {
        for (let x = 0; x < selectedShip.length; x += 1) {
          ...
          playerSquares[manualI + x].classList.add(
            `${selectedShipSelector.id}`
          );
        }
      ...
      if (manualY <= 10 - selectedShip.length) {
        for (let x = 0, y = 0; x < selectedShip.length; x += 1, y += 9) {
          ...
          playerSquares[manualI + (x + y)].classList.add(
            `${selectedShipSelector.id}`
          );
        }
    ...
    if (collision > 0) {
      Array.from(
        document.getElementsByClassName(selectedShipSelector.id)
      ).forEach((element) => {
  ...
  if (
    ...
    if (selectedShipSelector.classList.contains('horizontal')) {
      if (manualX <= 10 - selectedShip.length) {
        for (let x = 0; x < selectedShip.length; x += 1) {
          playerSquares[manualI + x].classList.remove(
            `${selectedShipSelector.id}`,
            'invalid'
          );
        }
      ...
      if (manualY <= 10 - selectedShip.length) {
        for (let x = 0, y = 0; x < selectedShip.length; x += 1, y += 9) {
          playerSquares[manualI + (x + y)].classList.remove(
            `${selectedShipSelector.id}`,
            'invalid'
          );
        }
